...
def lot_parti_plani_oner(
    ...
    for ihtiyac in recete_ihtiyaclari_getir(
        ...
        if kalan_parti > 0:
            ...
            raise ValueError(
                f'{ihtiyac["hammadde"]} için yeterli '
                f'hammadde stoku yok. '
                f'{parti_sayisi} parti ihtiyacı: '
                f'{float(ihtiyac["ihtiyac_kg"]):.3f} kg | '
                f'Mevcut kullanılabilir stok: '
                f'{toplam_kalan_kg:.3f} kg. '
                f'Önce DEPO KABUL kaydı oluşturun.'
            )
