...
def init_database():
    ...
    for ad, birim in hammaddeler:
        hammadde_exists = cur.execute("""
            SELECT 1
            FROM hammaddeler
            WHERE ad = ?
            LIMIT 1
        ...
        if hammadde_exists is None:
            cur.execute("""
                INSERT INTO hammaddeler (ad, birim)
                VALUES (?, ?)
            """, (
                ad,
                birim,
    ...
    if recete_exists is None:
        ...
        """, (
            ...
            None,
            (
                "2026 tarihsel başlangıç reçetesi / "
                "REDBOX OS geçiş kaydı"
            ),
            None,
        ))
    ...
    for hammadde_adi, miktar_kg in recete_kalemleri:
        ...
        recete_kalemi_exists = conn.execute("""
            SELECT 1
        ...
        if recete_kalemi_exists is None:
            conn.execute("""
    ...
    for ad_soyad, yetki_kodu in personel_yetki_seed:
        ...
        personel_yetki_exists = conn.execute("""
            SELECT 1
        ...
        if personel_yetki_exists is None:
            conn.execute("""
