...
class RedboxOS(ctk.CTk):
    ...
    def ana_sayfa(self):
        ...
        yil_uretim_kg = sum(
            ...
            if tarih_yili(row["uretim_tarihi"]) == yil
        )

        yil_sevkiyatlari = [
            row
            for row in sevkiyat_kayitlari
            if tarih_yili(row["sevkiyat_tarihi"]) == yil
        ]
        ...
        stok_500 = ambalaj_stok_toplami(500)
        stok_2500 = ambalaj_stok_toplami(2500)

        mamul_stoklar = mamul_stok_ozeti()

        stok_500["tam_koli"] = sum(
            row["tam_koli"]
            for row in mamul_stoklar
            if row["ambalaj_gram"] == 500
        )
        stok_500["acik_paket"] = sum(
            row["acik_paket"]
            for row in mamul_stoklar
            if row["ambalaj_gram"] == 500
        )

        stok_2500["tam_koli"] = sum(
            row["tam_koli"]
            for row in mamul_stoklar
            if row["ambalaj_gram"] == 2500
        )
        stok_2500["acik_paket"] = sum(
            row["acik_paket"]
            for row in mamul_stoklar
            if row["ambalaj_gram"] == 2500
        )
        ...
        mamul_data = [
            (
                "500 g MAMUL",
                (
                    f'{stok_500["tam_koli"]} koli\n'
                    f'{stok_500["acik_paket"]} açık paket\n'
                    f'{stok_500["toplam_paket"]} paket'
                )
            ),
            (
                "2.5 kg MAMUL",
                (
                    f'{stok_2500["tam_koli"]} koli\n'
                    f'{stok_2500["acik_paket"]} açık paket\n'
                    f'{stok_2500["toplam_paket"]} paket'
                )
            ),
            (
                "TOPLAM MAMUL",
