...
def paketlemeleri_aktar(
    ...
    for row in paketlemeler:
        ...
        grup = paketleme_gruplari.setdefault(
            uretim_tarihi,
            {
                "paketlenen_kg": 0.0,
                "paketleme_firesi": 0.0,
            },
        )

        grup["paketlenen_kg"] += float(
            row["paketlenen_kg"]
        )

        grup["paketleme_firesi"] += float(
            row["paketleme_firesi"]
        )

    print("")
    print(
        "=== PAKETLEME OPERASYON BAZLI "
        "KUTLE DENKLIGI ==="
    )
    ...
    for (
        ...
        toplam_paketlenen = float(
            grup["paketlenen_kg"]
        )

        toplam_fire = float(
            grup["paketleme_firesi"]
        )
        ...
        if abs(fark) > 0.000001:
            raise ValueError(
                "PAKETLEME OPERASYON KUTLE "
                "DENKLIGI KAPANMADI: "
                f"{uretim_tarihi} | "
                f"NET {net:.3f} KG | "
                f"PAKETLI "
                f"{toplam_paketlenen:.3f} KG | "
                f"FIRE {toplam_fire:.3f} KG | "
                f"FARK {fark:.3f} KG"
            )
    ...
    for row in paketlemeler:
        ...
        koli_ici_raw = row.get(
            "koli_ici_adet"
        )

        koli_ici = (
            int(koli_ici_raw)
            if koli_ici_raw is not None
            else None
        )
    ...
    for row in sevkiyatlar:
        ...
        if not uretim_tarihleri:
            ...
            for (
                ...
                if map_uretim_dt > sevkiyat_dt:
                    continue

                fifo_adaylari.append(
                    (
                        map_uretim_dt,
                        map_uretim_tarihi,
                    )
            ...
            uretim_tarihleri = [
                item[1]
                for item in fifo_adaylari
            ]

            if not uretim_tarihleri:
                raise ValueError(
                    "SEVKIYAT FIFO LOT ADAYI YOK: "
